...
2. mode="safety" : string The mode of digging. Can be "safety" or "strict". Strict mode mean that function will stopped if turtle did not dig, but moved (If turtle moves into the air in process of digging it stop function). Safety mode means that function will stop only if turtle did not move
##### Returns
